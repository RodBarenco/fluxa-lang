...
## v0.23 — refcounted strings: O(1) reads, uniform free semantics (current)

`str` becomes a **refcounted immutable buffer**, on one rule: `free` checks whether
any other name still points at the buffer — it drops the current name always, and
frees the heap only when the count reaches zero.

Each string buffer carries a hidden header `[rc | bytes...]`; `Value.as.string` still
points at the bytes, so every reader is unchanged. Because Fluxa strings are
immutable (mutation always produces a new buffer), sharing the pointer on read is
always safe:

- **write / produce** → new buffer, rc = 1
- **read** (identifier, `arr[i]`, `dyn[i]`, `inst.field`) → same buffer, rc + 1 —
  **O(1)**, size-independent, no copy
- **free / reassignment / function return** → rc − 1; heap freed at rc 0

Reference counts are atomic (GCC `__atomic`), so strings crossing `std.flxthread`
boundaries are correct with no copy rule.

What this fixes and changes:
- **Bug K is resolved in all contexts**: `str` field reassignment in module Blocks,
  and per-call `str` local leaks. A new frame teardown releases the callee's
  heap-owning slots (strings + owned arrays) at four sites — normal return, the TCO
  trampoline, and both VM→interpreter bridges (locals only in the bridges; parameter
  slots hold VM register pointers owned by the VM). `dyn` slots are left untouched by
  teardown, because parameter binding adopts the caller's `dyn` without pinning and
  unpinning would steal the caller's pin.
- **The array-element aliasing trap is gone** (see FLUXA_GUIDE §12.5): `str x =
  names[0]; free(x)` leaves the element intact, and reassigning a reader in a loop
  never double-frees. Same observable behavior as a copy, at pointer cost.
- **`free(field)` on a Block instance field** now releases the field and nils it (was
  rejected before); `free` on `prst` still rejects.
- **`std.cache` arena strings**: `arena_str`/`arena_concat` return an ordinary
  refcounted copy (the previous borrow-from-arena convention was unsafe under the new
  teardown). The API behaves identically; the arena is still bulk-released by
  `arena_reset`/`arena_drop`.
- Also fixed along the way: a per-call leak of every `prst` scalar string, and a
  self-assign reference leak in the scope layer.

Producer discipline is enforced by an exhaustive audit: every site that puts a string
into a `Value` allocates through the refcount allocator (the `val_string` constructor,
scope copy-ins, all stdlib string constructors, FFI writeback). The `prst` pool keeps
its plain-malloc internals behind a strict copy boundary — stack slots never adopt a
pool pointer.

Performance (measured): 1M element reads of a 512-byte string dropped from ~2.05 s to
~0.44 s versus a copy-on-read design, and read time is flat across 512 B / 4 KB /
16 KB (O(1) confirmed; a copying design scales linearly). The `int`-path benchmarks
are unchanged.

Validation: full `make test-all` green — unit suite 82/82 (including the new
`bugk_ownership` regression test with ASan no-per-call-leak gates), suite2 8/8
sections, libs, integration, and hardware-sim. Leak matrix (module/main × Block/fn)
is zero; the reference game reports byte-identical ASan totals across 300- and
700-frame budgets. Known remaining and tracked separately: bug A — const-pool strings
leak once per compiled `while` (constant residual, not per-iteration; `bytecode.c` is
protected).
