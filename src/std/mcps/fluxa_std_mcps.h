#ifndef FLUXA_STD_MCPS_H
#define FLUXA_STD_MCPS_H

/*
 * std.mcps — MCP client over HTTPS (TLS enforced)
 *
 * Identical to std.mcpc but forces HTTPS:
 *   CURLOPT_SSL_VERIFYPEER = 1
 *   Rejects plain http:// endpoints for Fluxa-lang
 *
...
typedef struct { char *data; size_t len; size_t cap; } McpsBuf;

static size_t mcps_write_cb(void *ptr, size_t size, size_t nmemb, void *ud) {
    McpsBuf *b = (McpsBuf *)ud;
    size_t n = size * nmemb;
    if (b->len + n + 1 > b->cap) {
...
static inline McpsClient *mcps_unwrap(const Value *v, ErrStack *err,
    ...
    if (v->type != VAL_DYN || !v->as.dyn || v->as.dyn->count < 1 ||
        ...
        snprintf(errbuf, sizeof(errbuf),
            "mcps.%s: invalid cursor — use mcp.connect() first", fn);
        errstack_push(err, ERR_FLUXA, errbuf, "mcp", line);
        *had_error = 1; return NULL;
    }
    return (McpsClient *)v->as.dyn->items[0].as.ptr;
}

/* ── JSON-RPC POST helper ────────────────────────────────────────── */
static char *mcps_post(McpsClient *c, const char *body,
                       ErrStack *err, int *had_error, int line) {
    char errbuf[280];
    CURL *curl = curl_easy_init();
    if (!curl) {
        errstack_push(err, ERR_FLUXA, "mcps: curl init failed", "mcp", line);
        *had_error = 1; return NULL;
    }

    McpsBuf buf = {NULL, 0, 0};
    ...
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, mcps_write_cb);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &buf);
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, 60L);
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "fluxa-mcps/1.0");
    ...
    if (res != CURLE_OK) {
        snprintf(errbuf, sizeof(errbuf), "mcps: %s", curl_easy_strerror(res));
        errstack_push(err, ERR_FLUXA, errbuf, "mcp", line);
        *had_error = 1;
        free(buf.data);
        return NULL;
...
static char *mcps_json_str(const char *json, const char *key) {
...
static int mcps_json_has_error(const char *json) {
    return json && strstr(json, "\"error\"") != NULL &&
           strstr(json, "\"result\"") == NULL;
}

/* ── Dispatch ────────────────────────────────────────────────────── */
static inline Value fluxa_std_mcps_call(const char *fn_name,
...
#define MCP_ERR(msg) do { \
    char _eb[512]; \
    snprintf(_eb, sizeof(_eb), "mcps.%s (line %d): %.400s", \
             fn_name, line, (msg)); \
    errstack_push(err, ERR_FLUXA, _eb, "mcp", line); \
    *had_error = 1; return mcps_nil(); \
} while(0)

#define NEED(n) do { \
    if (argc < (n)) { \
        snprintf(errbuf, sizeof(errbuf), \
            "mcps.%s: expected %d arg(s), got %d", fn_name, (n), argc); \
        errstack_push(err, ERR_FLUXA, errbuf, "mcp", line); \
        *had_error = 1; return mcps_nil(); \
    } \
} while(0)
...
#define GET_CLIENT(idx) \
    McpsClient *client = mcps_unwrap(&args[(idx)], err, had_error, line, fn_name); \
    if (!client) return mcps_nil();

    /* mcp.connect(url) → dyn */
    if (strcmp(fn_name, "connect") == 0) {
        NEED(1); GET_STR(0, url);
        McpsClient *c = (McpsClient *)malloc(sizeof(McpsClient));
        c->url    = strdup(url);
        c->token  = NULL;
        c->req_id = 1;
        return mcps_wrap(c);
    ...
    if (strcmp(fn_name, "list_tools") == 0) {
        ...
        char *resp = mcps_post(client, body, err, had_error, line);
        if (!resp) return mcps_nil();
        ...
        if (mcps_json_has_error(resp)) {
            char *emsg = mcps_json_str(resp, "message");
    ...
    if (strcmp(fn_name, "call_text") == 0) {
        ...
        char *text = mcps_json_str(resp, "text");
        Value result = mcps_str(text ? text : resp);
        free(text); free(resp);
        return result;
    }
...
#undef GET_CLIENT
    ...
    snprintf(errbuf, sizeof(errbuf), "mcps.%s: unknown function", fn_name);
    errstack_push(err, ERR_FLUXA, errbuf, "mcp", line);
    *had_error = 1;
    return mcps_nil();
}
...
FLUXA_LIB_EXPORT(
    ...
    toml_key  = "std.mcps",
    owner     = "mcps",
    call      = fluxa_std_mcps_call,
    rt_aware  = 0,
    cfg_aware = 0
)

#endif /* FLUXA_STD_MCPS_H */
