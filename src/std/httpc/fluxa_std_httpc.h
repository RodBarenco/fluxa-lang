#ifndef FLUXA_STD_HTTPC_H
#define FLUXA_STD_HTTPC_H

/*
 * std.httpc — HTTP client (libcurl) for Fluxa-lang
 *
...
#define HTTP_ERR(msg) do { \
    snprintf(errbuf, sizeof(errbuf), "httpc: %s", (msg)); \
    ...
    *had_error = 1; return http_nil(); \
    ...
    if (res != CURLE_OK) {
        snprintf(errbuf, sizeof(errbuf), "httpc: %s", curl_easy_strerror(res));
        errstack_push(err, ERR_FLUXA, errbuf, "http", line);
        *had_error = 1;
        free(buf.data);
        return http_nil();
...
/* ── Dispatch ────────────────────────────────────────────────────── */
static inline Value fluxa_std_httpc_call(const char *fn_name,
...
#define HTTP_ERR(msg) do { \
    snprintf(errbuf, sizeof(errbuf), "httpc.%s (line %d): %s", \
             fn_name, line, (msg)); \
    errstack_push(err, ERR_FLUXA, errbuf, "http", line); \
    *had_error = 1; return http_nil(); \
} while(0)
...
#define NEED(n) do { \
    if (argc < (n)) { \
        snprintf(errbuf, sizeof(errbuf), \
            "httpc.%s: expected %d arg(s), got %d", fn_name, (n), argc); \
        errstack_push(err, ERR_FLUXA, errbuf, "http", line); \
        *had_error = 1; return http_nil(); \
    } \
} while(0)
...
#undef GET_RESP
    ...
    snprintf(errbuf, sizeof(errbuf), "httpc.%s: unknown function", fn_name);
    errstack_push(err, ERR_FLUXA, errbuf, "http", line);
    *had_error = 1;
    return http_nil();
}
...
FLUXA_LIB_EXPORT(
    ...
    toml_key  = "std.httpc",
    owner     = "httpc",
    call      = fluxa_std_httpc_call,
    rt_aware  = 0,
    cfg_aware = 0
)

#endif /* FLUXA_STD_HTTPC_H */
